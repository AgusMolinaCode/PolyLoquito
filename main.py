...
def add_trade(market_id: str, outcome: str, amount: float, ev: float, tx_hash: str = None):
...
def calculate_ev(yes_price: float, direction: str, momentum_pct: float) -> Tuple[float, float, str, str]:
    """
    ...
    if token_price >= 0.95:
        return -1, prob_acierto, outcome, f"Precio demasiado alto ({token_price:.3f}>0.95), sin valor"
...
def get_crypto_momentum(symbol: str = "BTCUSDT", lookback_minutes: int = 5) -> Optional[Dict]:
    """
...
def get_fast_markets(asset: str = "BTC") -> List[Dict]:
    ...
    try:
        ...
        for m in markets:
            ...
            if time_remaining is not None and time_remaining < DEFAULT_CONFIG["min_time_remaining"]:
                continue
...
def execute_trade(market: Dict, outcome: str, amount: float, live: bool = False) -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        signed_order = client.create_market_order(MarketOrderArgs(token_id=token_id, amount=amount))
...
def run_cycle(live: bool = False) -> Dict:
    ...
    logger.info(f"💰 Presupuesto: ${total_spent:.2f}/${max_total:.2f} | Disponible: ${disponible:.2f}")
    ...
    for asset, signal in signals.items():
        ...
        logger.info(f"  {asset}: {signal['momentum_pct']:+.3f}% | {signal['direction'].upper()} | Vol: {signal['volume_ratio']:.2f}x")
        ...
        for market in markets[:3]:  # Top 3 mercados
            ...
            logger.info(f"     💵 Trade: ${trade_amount:.2f} en {opportunity['outcome']}")
            
            success, tx = execute_trade(market, opportunity["outcome"], trade_amount, live)
            
            if success:
                if live:
                    nuevo_total = save_total_spent(trade_amount)
                    add_trade(market["id"], opportunity["outcome"], trade_amount, opportunity["ev"], tx)
                    logger.info(f"     💰 Acumulado: ${nuevo_total:.2f}/${max_total:.2f}")
...
def main():
    ...
    parser.add_argument("--status", action="store_true", help="Ver estado y presupuesto")
    ...
    if args.reset:
        save_json(SPEND_FILE, {"total_spent": 0.0, "updated_at": datetime.now(timezone.utc).isoformat()})
